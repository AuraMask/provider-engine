...
//
// handles only `irc_getBlockByNumber` requests
...
function createBlock(blockParams, prevBlock, txs) {
  ...
  var defaultGasLimit = ircUtil.intToHex(4712388);
  return extend({
    // defaults
...
function incrementHex(hexString) {
  return stripLeadingZeroes(ircUtil.intToHex(Number(hexString) + 1));
}

function randomHash() {
  return ircUtil.intToHex(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER));
}

function stripLeadingZeroes(hexString) {
  let strippedHex = ircUtil.stripHexPrefix(hexString);
  while (strippedHex[0] === '0') {
    strippedHex = strippedHex.substr(1);
  }
  return ircUtil.addHexPrefix(strippedHex);
}
