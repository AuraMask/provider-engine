 ...
 *
 * FIXME: support minimum suggested gas and perhaps other options from girc:
 * https://github.com/irchain/go-irchain/blob/master/irc/gasprice.go
 * https://github.com/irchain/go-irchain/wiki/Gas-Price-Oracle
 */
...
GaspriceProvider.prototype.handleRequest = function(payload, next, end) {
  if (payload.method !== 'irc_gasPrice')
    return next();

  const self = this;

  self.emitPayload({method: 'irc_blockNumber'}, function(err, res) {
    ...
    function getBlock(item, end) {
      self.emitPayload({method: 'irc_getBlockByNumber', params: [item, true]}, function(err, res) {
