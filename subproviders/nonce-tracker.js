...
//   irc_getTransactionCount (pending only)
  ...
  switch (payload.method) {
    ...
    case 'irc_sendRawTransaction':
      ...
      next(function(err, result, cb) {
        ...
        var stripped = ircUtil.stripHexPrefix(rawTx);
        var rawData = new Buffer(ircUtil.stripHexPrefix(rawTx), 'hex');
        var tx = new Transaction(new Buffer(ircUtil.stripHexPrefix(rawTx), 'hex'));
        // extract address
        var address = '0x' + tx.getSenderAddress().toString('hex').toLowerCase();
        // extract nonce and increment
        var nonce = ircUtil.bufferToInt(tx.nonce);
        nonce++;
