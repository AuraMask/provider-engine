/*
 * Emulate 'irc_accounts' / 'irc_sendTransaction' using 'irc_sendRawTransaction'
 *
...
// Tx Signature Flow
//
// handleRequest: irc_sendTransaction
...
HookedWalletSubprovider.prototype.fillInTxExtras = function(txParams, cb) {
  ...
  if (txParams.gasPrice === undefined) {
    ...
    reqs.gasPrice = self.emitPayload.bind(self, {method: 'irc_gasPrice', params: []});
  }

  if (txParams.nonce === undefined) {
    // console.log("need to get nonce")
    reqs.nonce = self.emitPayload.bind(self, {method: 'irc_getTransactionCount', params: [address, 'pending']});
  }
...
function resemblesAddress(string) {
  const fixed = ircUtil.addHexPrefix(string);
  const isValid = ircUtil.isValidAddress(fixed);
  return isValid;
}
...
function resemblesData(string) {
  const fixed = ircUtil.addHexPrefix(string);
  const isValidAddress = ircUtil.isValidAddress(fixed);
