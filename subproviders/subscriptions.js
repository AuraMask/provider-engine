...
SubscriptionSubprovider.prototype.irc_subscribe = function(payload, cb) {
  const self = this;
...
SubscriptionSubprovider.prototype.irc_unsubscribe = function(payload, cb) {
  const self = this;
...
SubscriptionSubprovider.prototype._notificationHandler = function(hexId, subscriptionType, result) {
  ...
  // it seems that webu doesn't expect there to be a separate error event
  // so we must emit null along with the result object
  self.emit('data', null, {
    jsonrpc: '2.0',
    method: 'irc_subscription',
    params: {
      subscription: hexId,
      result: result,
    },
...
SubscriptionSubprovider.prototype.handleRequest = function(payload, next, end) {
  switch (payload.method) {
    case 'irc_subscribe':
      this.irc_subscribe(payload, end);
      break;
    case 'irc_unsubscribe':
      this.irc_unsubscribe(payload, end);
      break;
    default:
